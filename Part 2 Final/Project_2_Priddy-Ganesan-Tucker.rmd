...
The threshold selected was also 0.84 to keep the number of false positives at a minimum. At this threshold, the LDA model performs worse across all performance metrics and than logistic regression model, seen in *Table 3*. This seems to indicate that a linear decision boundary may not be the best fit for the data set being utilized. 
...
At the selected threshold of 0.85 on the testing data, the QDA model only has 2,003 false positives. However, the number of true positives predicted is 9,007 which results in 5,147 less pockets of refuges being rescued and the number of false negatives is 5,473 which is an increase of 1,578% more blue tarps being unidentified compared to the logistic regression model. 
...
When analyzing the threshold performance, we found that there were many thresholds which performed equally well, so we chose the highest threshold so as to conform to our other models. At this threshold of 0.85, we found that the model had 1,695 false positives, and a testing ROC AUC score of 0.976. 
...
```{r rf-autoplot, fig.height= 3, fig.width=6, fig.cap="Tune Results"}
...
```{r linear-svm, warning=FALSE, message=FALSE}
...
For the Linear SVM model, we tuned the cost and margin. Cost represents the cost of predicting a smaple within or on the wrong side of the margin while margin is the epsilon for the loss function. We decided to use Bayesian hyperparameter tuning for our model, with ten iterations.
...
The Polynomial SVM performs well relative to most of the other models, but is not strong enough to be our final model choice for identifying blue tarp locations. The model has very high accuracy and precision, matching all of the other models, and has one of the better values of specificity from cross-validation, at about 0.84. However, it performs worse than KNN and Random Forest.

```{r poly-svm, warning=FALSE, message=FALSE}
...
The model has 6,671 false positives and 195 false negatives. This compares well compared to most of the models, but does not perform as well as random forest. So, choosing this model would take us in the wrong direction in terms of saving the most displaced persons.
...
The model has 6,334 false positives and 5,069 false negatives. While the model performs better than the linear svm model in terms of having much less false positives, this accounts for a little more than 30% improvement compared to performing ~1509% worse than the linear svm model for false negatives. Every single false negative could mean less displaced persons saved, so this model is less attractive of a choice than previous models.
