...
parameters <- extract_parameter_set_dials(elasticnet_wf) %>% 
    update(
        penalty=penalty(c(-1, 1)),
        mixture=mixture(c(-1, 1))
    )
...
tune_results <- tune_grid(elasticnet_wf, 
                          resamples=resamples,
                          grid=grid_random(parameters, size=20), 
                          metrics=performance_metrics)
...
elastic_roc <- get_ROC_plot(elasticnet_model_final_fit, train_data, test_data, "Penalized Log Reg Model")
# Visualize ROC plots
